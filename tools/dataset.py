...
def create(reshape_size=[400,400],active_joint_absence=False):
    ...
    i=0
    img_set = np.zeros([len(joints),reshape_size[0],reshape_size[1],3])
    for img_path in sorted(glob.glob(os.path.join("./images/", '*.jpg'))):
        img = skimage.io.imread(img_path)
        img_set[i]=skimage.transform.resize(img,reshape_size)
        for j in range(14):
                if joints[i][j][0]==-1: joints[i][j][0]=-1
                else: joints[i][j][0]=joints[i][j][0]*(reshape_size[0]/img.shape[1])
                if joints[i][j][1]==-1: joints[i][j][1]=-1
                else: joints[i][j][1]=joints[i][j][1]*(reshape_size[1]/img.shape[0])
        i+=1
